...
export function trimTrailingZeros(bytes: Uint8Array): string {
    ...
    while (i >= 0 && bytes[i] === 0) { // Check for zeros from the end
        i--; // Move backwards until a non-zero byte is found or the start of the array is reached
    }
    // Create a new Uint8Array with the bytes up to the last non-zero byte
