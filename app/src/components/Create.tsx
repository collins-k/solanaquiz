...
export const CreateQuiz: FC<Props> = ({ createSuccess }) => {
    ...
    return (
        <button
            className="group m-2 btn-wide btn animate-pulse bg-gradient-to-br from-indigo-500 to-fuchsia-500 hover:from-white hover:to-purple-300 text-black"
            onClick={createQuiz} disabled={!ourWallet.publicKey}
        >
            <div className="hidden group-disabled:block">
                Wallet not connected
            </div>
            <span className="block group-disabled:hidden">Generate Quiz</span>
        </button>
    )
}
